...
our::Texture2D *our::texture_utils::empty(GLenum format, glm::ivec2 size)
{
    ...
    //  bind means that any following command will be applied to that texture
    texture->bind();
    // this function used to allocate a storage for 2D texture
    // GL_TEXTURE_2D --> means that the texture that will be allocated will be of 2D array
    // 1 --> number of mipmaps levels
    // format --> the number of color components ans the size of each other ex(GL_RGBA8 and size UINT8)
